	...
	public static Inventory getRecipes(Player p, List<Recipe> available, int page) {
		...
		for(int i = page*9*3; i < page*9*3+9*3; i++) {
			...
			recipes.setItem(i-page*9*3,new ItemBuilder(r.getTarget().clone())
					.glow(crafter.getTarget().getTarget().getType()==r.getTarget().getType()&&crafter.getTarget().getTarget().getDurability()==r.getTarget().getDurability()&&crafter.getTarget().getTarget().equals(r.getTarget()))
					.setLore(lore)
					.build());
		}
		
		recipes.setItem(9*4-3, new ItemBuilder(Material.SIGN).setName(Messages.ITEM_SEARCH_NAME).setLore(Messages.ITEM_SEARCH_LORE.replaceFirst("%amount%", ""+available.size())).build());
		recipes.setItem(9*3+2, new ItemBuilder(Material.REDSTONE_TORCH_ON).setName(Messages.ITEM_RESETSEARCH_NAME).build());
